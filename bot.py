...
def main() -> None:
    ...
    if mode == "webhook":
        ...
        if secret and not re.fullmatch(r"[A-Za-z0-9_-]{1,256}", secret):
            raise RuntimeError(
                "WEBHOOK_SECRET chứa ký tự không hợp lệ. Telegram chỉ cho phép chữ, số, "
                "'_' và '-' (KHÔNG được có '/', '+', '=', ':', v.v.) - giá trị Render tự "
                "sinh bằng generateValue đôi khi ra dạng base64 có '/'/'=' nên không dùng "
                "được. Tự đặt lại 1 chuỗi khác trong Render -> Environment, ví dụ chạy "
                "`python3 -c \"import secrets; print(secrets.token_urlsafe(32))\"` rồi "
                "dán kết quả vào."
            )
        path = "webhook"
